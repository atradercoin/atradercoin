...
//    miningThreads = nLimitProcessors;
    ...
    if (mapArgs["-rpcserver"] != "" )
        miningServer = QString::fromStdString(mapArgs["-rpcserver"]);
    if (mapArgs["-rpcport"] != "")
        miningPort = QString::fromStdString(mapArgs["-rpcport"]);
    if (mapArgs["-rpcuser"] != "" )
        miningUsername = QString::fromStdString(mapArgs["-rpcuser"]);
    if (mapArgs["-rpcpassword"] != "" )
        miningPassword = QString::fromStdString(mapArgs["-rpcpassword"]);
...
{
    printf("setMiningType %d mining %d thread% hashrate %d\n", type, mining, threads, hashrate);
